...
function App({dispatch,basketTicket,basketHotel,basketTicketBuy,basketHotelBuy}) {
  ...
  useEffect(()=>{
    ...
    fetch("http://localhost:2605/tickets").then((a)=>a.json()).then((a)=>{
      dispatch({
        type:"SET_PRODUCTS",
        payload:a,
      })
  ...
  useEffect(()=>{
    fetch("http://localhost:2605/hotels").then((a)=>a.json()).then((a)=>{
      dispatch({
        type:"SET_HOTELS",
        payload:a,
      })
...
<Favicon url="https://www.freeiconspng.com/thumbs/travel-icon-png/plane-travel-flight-tourism-travel-icon-png-10.png" />

      {
        loading ?(
          ...
          {pathname !== "/not-found" && pathname !== "/" && pathname!=="/flight-search" && pathname!=="/hotel-search" && pathname!=="/login" && pathname!=="/sign-up" && pathname!=="/about" && pathname!=="/contact" && <SecondNavbar/>}
          <Routes>
            ...
            <Route  path='/booking-detailsCard/:id' element={ <BookingDetailsCard/>}/>
            <Route  path='/hotel-listing-one' element={ <HotelListingone/>}/>
            <Route  path='/hotel-listing-two/:id' element={ <HotelListingtwo/>}/>
            <Route  path='/hotels-booking-details/:id' element={  <HotelsBookingDetails/>}/>
            ...
            <Route path='/hotel-detailsCard/:id' element={<HotelDetailsCard/>}/>
